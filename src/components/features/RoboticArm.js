...
const RoboticArm = () => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      bolt.position.set(Math.cos(angle) * 0.75, 0.12, Math.sin(angle) * 0.75);
      baseGroup.add(bolt);
    }
    ...
    const debugText = addDebugText(
      'Inverse Kinematics: Poetry in Motion',
    ...
    const forwardKinematics = angles => {
      ...
      const shoulderMatrix = new THREE.Matrix4()
        .makeTranslation(shoulderPos.x, shoulderPos.y, shoulderPos.z)
      ...
      const elbowMatrix = new THREE.Matrix4()
        .makeTranslation(elbowPos.x, elbowPos.y, elbowPos.z)
        .multiply(new THREE.Matrix4().makeRotationX(a3));

      const wristPos = new THREE.Vector3(0, 0, armLength3);
      const wristPitchMatrix = new THREE.Matrix4()
        .makeTranslation(wristPos.x, wristPos.y, wristPos.z)
        .multiply(new THREE.Matrix4().makeRotationX(a4));

      const wristRollPos = new THREE.Vector3(0, 0, 0);
      const wristRollMatrix = new THREE.Matrix4()
        .makeTranslation(wristRollPos.x, wristRollPos.y, wristRollPos.z)
        .multiply(new THREE.Matrix4().makeRotationZ(a5));

      const wristYawPos = new THREE.Vector3(0, 0, 0);
      const wristYawMatrix = new THREE.Matrix4()
        .makeTranslation(wristYawPos.x, wristYawPos.y, wristYawPos.z)
      ...
      const handMatrix = new THREE.Matrix4()
        .makeTranslation(handPos.x, handPos.y, handPos.z)
        .multiply(new THREE.Matrix4().makeRotationZ(a7));

      const gripperPos = new THREE.Vector3(0, 0, armLength5);
      const gripperMatrix = new THREE.Matrix4().makeTranslation(
        gripperPos.x,
        gripperPos.y,
        gripperPos.z
      );
    ...
    const planPath = (startPos, endPos, numPoints = 20) => {
      ...
      for (let i = 0; i <= numPoints; i++) {
        ...
        point.x =
          startPos.x * mt3 + 3 * midPoint.x * mt2 * t + 3 * midPoint.x * mt * t2 + endPos.x * t3;
        point.y =
          startPos.y * mt3 + 3 * midPoint.y * mt2 * t + 3 * midPoint.y * mt * t2 + endPos.y * t3;
        point.z =
          startPos.z * mt3 + 3 * midPoint.z * mt2 * t + 3 * midPoint.z * mt * t2 + endPos.z * t3;
